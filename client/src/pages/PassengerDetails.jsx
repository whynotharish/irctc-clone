...
export default function PassengerDetails() {
  ...
  const seatIds = (params.get("seats") || "").split(",").filter(Boolean).map(Number);
  const sessionId = params.get("sessionId");

  const [passengers, setPassengers] = useState(
    seatIds.map(() => ({ name: "", age: "", gender: "M" }))
  );
  ...
  const fareEach = 1450;
  const total = seatIds.length * fareEach;
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-2xl mx-auto px-6 pb-16">
        <div className="mt-4 mb-8 bg-(--color-paper-raised) border border-(--color-line) rounded-2xl px-6 py-4">
          <JourneyLine
            compact
            progress={0.66}
            stations={[{ code: "Search", label: "" }, { code: "Seats", label: "" }, { code: "Pay", label: "" }]}
          />
        </div>
        ...
        <form onSubmit={handlePay} className="space-y-4">
          ...
          <motion.button
            ...
            type="submit"
            disabled={submitting}
            className="w-full py-4 rounded-xl bg-(--color-ink) text-white font-medium flex items-center justify-center gap-2 hover:bg-(--color-ink)/90 transition-colors disabled:opacity-60"
          >
            <CreditCard className="w-4 h-4" />
            {submitting ? "Processing payment…" : `Pay ₹${total.toLocaleString("en-IN")} & confirm`}
          </motion.button>
        </form>
      </main>
    </div>
